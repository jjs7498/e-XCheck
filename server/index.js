...
app.post("/predict-image", async (req, res) => {
  ...
  try {
    ...
    const filteredPredictionsResult = filterPredictions(
    ...
    const jimpImages = await Promise.all(
      filteredPredictionsResult.map(async (prediction) => {
        ...
        return (
          "data:image/jpeg;base64," + croppedImageBuffer.toString("base64")
        );
      })
    );
    ...
    const productInfo = await Promise.all(
      filteredPredictionsResult.map(async (prediction, index) => {
        //console.log(prediction.tagName);
        const productRef = db
          .collection("products")
          .doc(prediction.tagName ? prediction.tagName : "");
